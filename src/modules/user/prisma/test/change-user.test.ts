...
describe('changing mutable user properties', () => {
  ...
  it('rejects if user targeted by display name change does not exist', async () => {
    ...
    await expect(async () => await changeUserDisplayName(falseId, newDisplayName)).rejects.toThrowError('user_does_not_exist')
  })
  ...
  it('rejects if new display name is too long', async () => {
    ...
    await expect(async () => await changeUserDisplayName(user.id, newDisplayName)).rejects.toThrowError('name_too_long')
  })
  ...
  it('rejects if new display name is too short', async () => {
    ...
    await expect(async () => await changeUserDisplayName(user.id, newDisplayName)).rejects.toThrowError('name_too_short')
  })
