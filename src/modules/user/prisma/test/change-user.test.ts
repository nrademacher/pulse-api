...
describe('changing mutable user properties', () => {
  ...
  it('rejects if user targeted by bio change does not exist', async () => {
    ...
    await expect(
      async () => await changeUserBio(falseId, newBio),
  ...
  it('rejects if new bio exceeds maximum length', async () => {
    ...
    await expect(
      async () => await changeUserBio(user.id, newBio),
    ).rejects.toThrowError('bio_too_long');
  });
  ...
  it('changes a user display name', async () => {
    ...
    const userWithNewDisplayName = await changeUserDisplayName(
      user.id,
      newDisplayName,
    );

    expect(userWithNewDisplayName).toHaveProperty(
      'displayName',
      newDisplayName,
    );
  });
  ...
  it('rejects if user targeted by display name change does not exist', async () => {
    ...
    await expect(
      async () => await changeUserDisplayName(falseId, newDisplayName),
    ).rejects.toThrowError('user_does_not_exist');
  });
  ...
  it('rejects if new display name is too long', async () => {
    ...
    await expect(
      async () => await changeUserDisplayName(user.id, newDisplayName),
    ).rejects.toThrowError('name_too_long');
  });
  ...
  it('rejects if new display name is too short', async () => {
    ...
    await expect(
      async () => await changeUserDisplayName(user.id, newDisplayName),
    ).rejects.toThrowError('name_too_short');
  });
  ...
  it('rejects if user targeted by role change does not exist', async () => {
    ...
    await expect(
      async () => await changeUserRole(falseEmail, newRole),
    ).rejects.toThrowError('user_does_not_exist');
  });
});
